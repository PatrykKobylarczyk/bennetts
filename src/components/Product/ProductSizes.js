...
const ProductSizes = ({selectedSize, selectSizeHandler}) => {
    ...
    const sizes = sizeTable.map((size, i) => {
        return (
            <li
                id={i}
                key={i}
                className={`h-8 text-xs flex justify-center items-center border-[1px]  ${size==selectedSize ? 'border-gray-400' : 'border-gray-200' } rounded-lg hover:border-gray-400  hover:cursor-pointer`}
