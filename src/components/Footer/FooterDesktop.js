...
const FooterDesktop = () => {
    ...
    const footerMenuPanels = footerMenu.map((panel, i) => {
        ...
        const subMenuPanel = panel.subMenu && panel.subMenu.map(item => <li className='capitalize text-xs font-extralight text-white text-opacity-30 hover:text-opacity-100 transition duration-150 mt-3'><a href='#' key={item.name}>{item.name}</a></li>)
